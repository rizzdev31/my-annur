...
async function load() { loading.value = true; try { d.value = (await api.get('/absensi', { params: { bulan: bulan.value } })).data.data } catch (_) {} finally { loading.value = false } }
onMounted(load)
</script>
<template>
    <div>
        ...
        <template v-else-if="d">
            ...
            <div v-if="!d.rows.length" class="pt-10 text-center text-sm text-gray-400">Belum ada data KBM bulan ini.</div>
            <ul v-else class="space-y-2">
                <li v-for="(r, i) in d.rows" :key="i" class="flex items-center justify-between bg-white rounded-xl border border-gray-100 px-3 py-2.5">
                    <div class="min-w-0"><p class="text-sm font-semibold text-gray-800 truncate">{{ r.mapel }}</p><p class="text-[10px] text-gray-400">{{ r.tanggal }} · {{ r.guru }}</p></div>
