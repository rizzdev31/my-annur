...
const rekap = computed(() => {
    const r = { hadir: 0, telat: 0, alpha: 0 }
...
function tandaiSemua(v) { if (!terkunci.value) santri.value.forEach(s => s.status = v) }
    ...
    <div>
        ...
        <template v-else-if="d">
            ...
            <div class="flex items-center justify-between mb-2">
                ...
                <button v-if="!terkunci && !belumAbsenMengajar" @click="tandaiSemua('hadir')" class="text-xs font-bold text-[#0C78FF]">Semua Hadir</button>
            </div>
                ...
                <li v-for="s in santri" :key="s.santri_id" class="rounded-2xl bg-white border border-gray-100 p-3">
                    <div class="flex items-center justify-between gap-2">
                        <div class="min-w-0">
                            ...
                            <p v-if="s.nip" class="text-[10px] text-gray-400">NIS {{ s.nip }}</p>
                        </div>
                        <div class="flex gap-1 shrink-0">
                            <button v-for="st in STATUS" :key="st.v" @click="setStatus(s, st.v)" :disabled="terkunci || belumAbsenMengajar"
                                class="w-9 py-1.5 rounded-lg text-[11px] font-bold transition"
        ...
        <BottomSheet v-model="konfirm" title="Simpan Absensi Santri?" subtitle="Setelah disimpan, absensi terkunci & tidak bisa diubah.">
            <div class="flex gap-1.5 text-[12px] font-bold mb-4">
